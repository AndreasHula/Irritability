...
n = 10;
for j = 1:(length(X))
fid =fopen(['BrooksFlexRepOld' int2str(X(j)) '.bin'],'r');
for i = 1:n
...
fid =fopen(['BrooksFlexRepOld9.bin'],'r');
for i = 1:3
    InvestorLikelihoods(1,i+(j-1)*n)=fread(fid,1,'double');
    InvestorToM(1,i+(j-1)*n)=fread(fid,1,'int32');
    InvestorGuilt(1,i+(j-1)*n)=fread(fid,1,'int32');
...
n = 10;
for j = 1:(length(X))
fid =fopen(['BrooksFlexRepOld' int2str(X(j)) 'Level0.bin'],'r');
for i = 1:n
...
fid =fopen(['BrooksFlexRepOld9Level0.bin'],'r');
for i = 1:3
    HInvestorLikelihoods(1,i+(j-1)*n)=fread(fid,1,'double');
    HInvestorToM(1,i+(j-1)*n)=fread(fid,1,'int32');
    HInvestorGuilt(1,i+(j-1)*n)=fread(fid,1,'int32');
...
for i = 1:93
   [WInvestorLikelihoods(i), ipos(i)] = min([InvestorLikelihoods(i) HInvestorLikelihoods(i)]);
    if ipos(i) == 1
   ...
   % InvestorLikelihoods(i) = MInvestorLikelihoods(c+i);    
    ...
    [WTrusteeLikelihoods(i),pos(i)] = min([TrusteeLikelihoods(i) HTrusteeLikelihoods(i)]);
    if pos(i) == 1
