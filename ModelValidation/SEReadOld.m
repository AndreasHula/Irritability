...
n = 93;
for j = 1:(length(X))
fid =fopen(['BrooksOldRepFlexible.bin'],'r');
for i = 1:n
...
n = 93;
for j = 1:(length(X))
fid =fopen(['BrooksOldRepFlexibleLevel0.bin'],'r');
for i = 1:n
...
for i = 1:93
   [GInvestorLikelihoods(i), ipos(i)] = min([InvestorLikelihoods(i) HInvestorLikelihoods(i)]);
    if ipos(i) == 1
   ...
   % InvestorLikelihoods(i) = MInvestorLikelihoods(c+i);    
    ...
    [GTrusteeLikelihoods(i),pos(i)] = min([TrusteeLikelihoods(i) HTrusteeLikelihoods(i)]);
    if pos(i) == 1
